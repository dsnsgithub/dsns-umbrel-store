...
def download():
    ...
    if fmt != 'video':
        ...
        for f in info.get('formats', []):
            ...
            if f.get('vcodec') != 'none':
                # skip combined formats for audio request; prefer audio-only stream
                continue
            score = (f.get('abr') or 0)
            if best_audio is None or score > best_audio[0]:
                best_audio = (score, f)
        if best_audio:
            prog_format = best_audio[1]
    ...
    if prog_format and prog_format.get('url'):
        ...
        logging.info("Streaming direct format %s from %s", prog_format.get('format_id'), direct_url)
        ...
        return Response(stream_direct(), mimetype=mimetype, headers={
    ...
    logging.info("Falling back to yt-dlp subprocess: %s", " ".join(shlex.quote(c) for c in cmd))

    def generate_from_proc():
        # Start the yt-dlp process and stream stdout.
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1024*1024*4)
        try:
            while True:
    ...
    disposition = f"attachment; filename*=UTF-8''{quote(filename)}"
    return Response(generate_from_proc(), mimetype=mimetype, headers={
        'Content-Disposition': disposition
    })
